...
class P0fRecord(models.Model):
    ...
    uptime_sec = models.IntegerField(null=True, blank=True, help_text="Calculated system uptime in seconds, None if not available")
    up_mod_days = models.IntegerField(null=True, blank=True, help_text="Uptime wrap-around time in days")
    last_nat = models.DateTimeField(null=True, blank=True, help_text="Time of the most recent detection of IP sharing, None if never detected")
    last_chg = models.DateTimeField(null=True, blank=True, help_text="Time of the most recent OS mismatch (e.g multiboot or IP reuse)")
    distance = models.IntegerField(null=True, blank=True, help_text="System distance (from TTL). None if no data")
    bad_sw = models.CharField(max_length=1, choices=BAD_SW, null=True, help_text="Whether user-agent and/or Server string are accurate")
    ...
    http_name = models.CharField(max_length=32, null=True, blank=True, help_text="Name of the most recently identified HTTP application")
    http_flavor = models.CharField(max_length=32, null=True, blank=True, help_text="Flavor of the most recently identified HTTP application")
